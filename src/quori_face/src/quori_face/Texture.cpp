...
Texture::Ptr Texture::create(std::size_t rows, std::size_t cols, float *const data)
{
  ...
  QUORI_FACE_TRACE(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_BORDER));
  QUORI_FACE_TRACE(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_BORDER));
